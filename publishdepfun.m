function new_doc=publishdepfun(func,opts,add_fun,rm_fun)
%%
...
funcId = find(contains(out, func)); 
out_doc{1}=publish(out{funcId},opts);
...
for i=1:length(out)
    if i ~= funcId
        out_doc{end+1}=publish(out{i},options);
    end
end
